...
# TODO WELD_CORE_VERSION a EAP_VERSION

JSF_VERSION=$(echo $JSF_MODULE | awk -F- '{print $NF}')
JSF_NAME=${JSF_MODULE/-$JSF_VERSION/}

# Static paths
WELD_CORE_PATH="$EAP_HOME/modules/system/layers/base/org/jboss/weld/core/main"
WELD_CORE_VERSION=$(awk -F "-" '{ print $3 }')
...
MOJARRA_API_MODULE="<?xml version="1.0" encoding="UTF-8"?> <module xmlns="urn:jboss:module:1.1" name="org.jboss.as.jsf-injection" slot="${JSF_MODULE}"> <properties> <property name="jboss.api" value="private"/> </properties> <resources> <resource-root path="wildfly-jsf-injection-${EAP_VERSION}.jar"/> <resource-root path="weld-core-jsf-${WELD_CORE_VERSION}.jar"/> </resources> <dependencies> <module name="com.sun.jsf-impl" slot="${JSF_MODULE}"/> <module name="javax.api"/> <module name="org.jboss.as.web-common"/> <module name="javax.servlet.api"/> <module name="org.jboss.as.ee"/> <module name="javax.enterprise.api"/> <module name="org.jboss.logging"/> <module name="org.jboss.weld.core"/> <module name="javax.faces.api" slot="${JSF_MODULE}"/> </dependencies> </module> "
...
installModule() {
    ...
    wget -U "Dummy User Agent" $API_JAR_URL
    wget -U "Dummy User Agent" $IMPL_JAR_URL
